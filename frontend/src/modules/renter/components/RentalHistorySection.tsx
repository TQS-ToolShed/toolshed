 ...
 * This is the non-modal version of RentalHistoryModal for inline display
 */
export const RentalHistorySection = () => {
  const { user } = useAuth();
    ...
    <>
      <div className="bg-background border border-border rounded-xl shadow-sm">
        <div className="px-5 py-4 border-b border-border">
          ...
          <p className="text-sm text-muted-foreground">Your past bookings</p>
        </div>
                      ...
                      <div className="flex flex-col gap-2 shrink-0">
                        {!booking.conditionStatus && (
                          <button
                            ...
                            className="px-3 py-1.5 rounded-md text-xs font-medium whitespace-nowrap transition-colors bg-purple-600 text-white hover:bg-purple-700"
                          >
                            Report Condition
                          </button>
                        )}
                        {booking.depositStatus === 'REQUIRED' && (
                          <button
                            ...
                            className="px-3 py-1.5 rounded-md text-xs font-medium whitespace-nowrap transition-colors bg-amber-600 text-white hover:bg-amber-700 disabled:opacity-50"
                          >
                            {isPayingDeposit === booking.id ? 'Paying...' : `Pay €${booking.depositAmount?.toFixed(2)}`}
                          </button>
                        )}
                        <button
                          onClick={() => {
